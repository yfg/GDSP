...
\version\verbatim $Id: balance.c 17627 2014-09-10 00:27:58Z dominique $ \endverbatim
*/
...
void Bnd2WayBalance(ctrl_t *ctrl, graph_t *graph, real_t *ntpwgts)
{
  ...
  nbnd = graph->nbnd;
  my_irandArrayPermute_r(nbnd, perm, nbnd/5, 1, &ctrl->curseed);
  for (ii=0; ii<nbnd; ii++) {
    i = perm[ii];
...
void General2WayBalance(ctrl_t *ctrl, graph_t *graph, real_t *ntpwgts)
{
  ...
  my_irandArrayPermute_r(nvtxs, perm, nvtxs/5, 1, &ctrl->curseed);
...
void McGeneral2WayBalance(ctrl_t *ctrl, graph_t *graph, real_t *ntpwgts)
{
  ...
  nbnd = graph->nbnd;
  my_irandArrayPermute_r(nvtxs, perm, nvtxs/10, 1, &ctrl->curseed);
  for (ii=0; ii<nvtxs; ii++) {
    i = perm[ii];
