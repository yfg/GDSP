...
namespace push_relabel_highest
{
    ...
    public:
        max_flow_instance ( vector<vector<cached_edge<T, U>>> graph, T source, T sink )
                ...
                _labels ( new label_info[ _residual_network . size () + 1 ] ),
                _vertices ( new vertex[ _residual_network . size () ] ),
        ...
        // auto steal_network ( )
        vector<vector<cached_edge<T, U>>> steal_network ( )
        {
    ...
    private:
        ...
        inline vertex* get_active_vertex ( )
        {
